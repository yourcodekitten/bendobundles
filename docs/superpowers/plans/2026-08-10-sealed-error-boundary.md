...
- **THIS BOX CANNOT LINK `cargo test` FOR THE HEAVY CRATES** — a workspace test link needs ≥1638M (measured 2026-08-07). Local gate is `cargo check --workspace --all-targets -j 1` (413M peak) and `cargo clippy … -j 1` (468M peak). The census test lives in `domain` (3 direct deps) specifically because that is the one crate whose test binary might link here; **if it OOMs, say so and take the evidence from CI — never report a local pass you did not see.**
